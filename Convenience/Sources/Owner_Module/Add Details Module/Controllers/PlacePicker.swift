...
extension PlacePicker{
    func showLocationAlert() {
        let alert = UIAlertController(title: "Location is disabled", message: "We need access to your location to show you relevant search result, Please click on Settings to allow location.", preferredStyle: .alert)
        alert.addAction(UIAlertAction(title: "Cancel", style: .cancel, handler: nil))
        alert.addAction(UIAlertAction(title: "Settings", style: .default, handler: { _ in
            let url = URL(string: UIApplication.openSettingsURLString)!
            UIApplication.shared.open(url, options: [:], completionHandler: nil)
